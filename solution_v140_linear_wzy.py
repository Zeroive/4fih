...
def _v36_calib_comp_gate(w_orig, acts, smooth, had, wq_trans, block_map):
    """Require the fixed map to improve quantized calibration outputs consistently."""
    if block_map is None or not acts:
        return False
    base_sum = 0.0
    cand_sum = 0.0
    worst = 0.0
    # Same deterministic bounded pilot philosophy as V31, but compare V31 vs V36.
    iw = _v31_even(w_orig.shape[0], min(48, w_orig.shape[0]), w_orig.device)
    wq_p = wq_trans[iw]
    w_p = w_orig[iw]
    for a in acts[:4]:
        ...
        if had:
            at = _fwht64_v31(at)
        _, aq0 = _quantize_tensor_self_mse(at, return_dequant=True)
        e0 = float((aq0 @ wq_p.t() - ref).square().mean().item())
        atc = _v36_apply_block_map(at, block_map)
        _, aq1 = _quantize_tensor_self_mse(atc, return_dequant=True)
        e1 = float((aq1 @ wq_p.t() - ref).square().mean().item())
        base_sum += e0
        cand_sum += e1
        worst = max(worst, e1 / max(e0, 1.0e-20))
    ratio = cand_sum / max(base_sum, 1.0e-20)
    return ratio <= _V36_COMP_POOLED_GATE and worst <= _V36_COMP_WORST_GATE
...
def _v40_local_product_error(a,w,aq,wq):
    nb=a.shape[-1]//64
    a4=a.float().reshape(a.shape[0],nb,64);w4=w.float().reshape(w.shape[0],nb,64)
    aq4=aq.float().reshape(aq.shape[0],nb,64);wq4=wq.float().reshape(wq.shape[0],nb,64)
    ref=torch.einsum('rbk,sbk->brs',a4,w4);out=torch.einsum('rbk,sbk->brs',aq4,wq4)
    return (out-ref).square().mean(dim=(1,2))
...
def _v64_gate_lr(w_orig, acts, state, wq, block_map, lr):
    if lr is None or not acts:
        return None
    Q,R,_ = lr
    bs = cs = 0.0; worst = 0.0
    for a in acts[:4]:
        ia = _v31_even(a.shape[0], min(24,a.shape[0]), a.device)
        ap=a[ia]; ref=ap@w_orig.t(); at=_v51_apply(ap,state,False)
        y0=_v36_apply_block_map(at,block_map); y1=_v37_apply_comp(at,block_map,Q,R)
        e0=float((y0@wq.t()-ref).square().mean().item()); e1=float((y1@wq.t()-ref).square().mean().item())
        bs+=e0; cs+=e1; worst=max(worst,e1/max(e0,1e-20))
    return lr if cs <= bs*0.995 and worst <= 1.01 else None

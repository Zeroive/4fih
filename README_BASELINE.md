...
### 当前 solution.py：E6M2 全值表与动态候选值

- 先按 6-bit exponent、2-bit mantissa 的编码空间生成全部 255 个正有限 E6M2 值；最大 exponent 的 mantissa code 3 为 NaN，不进入表中。
- scale 搜索不再传递或遍历 offset，也不使用人为的倍数阈值；每个 anchor 直接从完整值表收集实际候选值：从前一个合法编码开始，覆盖一个由 2-bit mantissa 编码数自然确定的完整编码段，并按值表两端动态裁剪。
- 固定随机输入覆盖常规值、极小值和超大值时，所有 HiF4 参数与 `v171_` 逐项完全一致。
- 完整真实 GQA 自检仍为 7/12；Attention 五项 MSE 与 `v171_` 逐项一致，Linear 平均 MSE 仍约 `1.64686e-3`。
- 本次平均校准时延 `2759.14 ms`、平均动态时延 `1132.54 ms`，相对 `v171_` 未见明显增加；这是等价的通用格式重构，因此不单独保存新版本快照。

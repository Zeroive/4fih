"""Tail-Aware HiFloat4, adapted only to the six-function HiF4 demo interface.

Percentile activation statistics replace hard maxima in the paper's equivalent channel mask.
No techniques from the other solution variants are included.
======================== Linear ========================
[Linear][Group 0] calibration: PASSED [167.45ms] (W=(8192, 2048), num_calib=5)
[Linear][Group 0][Test 0] activation: FAILED [1.67ms] (W=(8192, 2048), A=(10, 2048))
      MatMul MSE 1.3633e-02 exceeds threshold 0.001
[Linear][Group 0][Test 1] activation: FAILED [3.62ms] (W=(8192, 2048), A=(128, 2048))
      MatMul MSE 1.4576e-02 exceeds threshold 0.001
[Linear][Group 0][Test 2] activation: FAILED [8.00ms] (W=(8192, 2048), A=(512, 2048))
      MatMul MSE 1.1500e-02 exceeds threshold 0.001
[Linear][Group 0][Test 3] activation: FAILED [11.75ms] (W=(8192, 2048), A=(1024, 2048))
      MatMul MSE 9.5841e-03 exceeds threshold 0.001
[Linear][Group 0][Test 4] activation: FAILED [11.03ms] (W=(8192, 2048), A=(1024, 2048))
      MatMul MSE 1.0363e-02 exceeds threshold 0.001

====================== Attention ======================
[Attention][Group 0] calibration: PASSED [0.00ms] (q_heads=16, kv_heads=2, head_dim=256, num_calib=5)
[Attention][Group 0][Test 0] FAILED [7.22ms] (Q=(10, 4096), K=(10, 512), V=(10, 512))
      Attention MSE 1.0388e-03 exceeds threshold 0.001
[Attention][Group 0][Test 1] PASSED (MSE=4.0943e-04) [22.96ms] (Q=(128, 4096), K=(128, 512), V=(128, 512))
[Attention][Group 0][Test 2] PASSED (MSE=2.8481e-04) [51.50ms] (Q=(512, 4096), K=(512, 512), V=(512, 512))
[Attention][Group 0][Test 3] PASSED (MSE=2.1662e-04) [132.52ms] (Q=(1024, 4096), K=(1024, 512), V=(1024, 512))
[Attention][Group 0][Test 4] PASSED (MSE=2.3384e-04) [92.28ms] (Q=(1024, 4096), K=(1024, 512), V=(1024, 512))
"""
...
def hif4_calibration_and_quantize_weight(weight_quant, weight_scale, calib_activation_list):
    ...
    for activation_quant, activation_scale in calib_activation_list:
        ...
        samples.append(A.abs())
    if samples:
        act_stat = torch.quantile(torch.cat(samples, dim=0), TAIL_PERCENTILE, dim=0)
    else:
        act_stat = torch.ones(channels, device=W.device)
    weight_stat = W.abs().amax(dim=0)
    mask = weight_stat.clamp_min(EPS).pow(TAIL_ALPHA) / act_stat.clamp_min(EPS).pow(1.0 - TAIL_ALPHA)
    return {
        "weight_params": _hif4_direct(W / mask.unsqueeze(0)),
        "activation_state": {"tail_mask": mask.detach().cpu()},
    }
...
def hif4_dynamic_quantize_activation(activation_quant, activation_scale, activation_state):
    ...
    mask = activation_state["tail_mask"].to(A.device)
    return _hif4_direct(A * mask)

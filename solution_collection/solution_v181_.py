...
def _safe147_geometry(w, acts):
    ...
    arms = torch.stack([a.square().mean(0) for a in acts], 0).mean(0).sqrt()
    wrms = w.square().mean(0).sqrt()
    smooth = _v31_smooth(arms, wrms, 0.5)
    apre = [a.float() * smooth for a in acts]
    mass = _safe139_sample_rms(apre, k)
    perm = _safe139_massdiff_perm(mass, 8)
...
def hif4_calibration_and_quantize_weight(
    ...
    Dynamic activation optimization uses Wq^T Wq plus a cross-target gradient
    that lets activation error compensate part of the weight error.
    """
    base = _v156_linear(
        weight_quant,
        weight_scale,
    ...
    if k % 64 == 0:
        ...
        cross = wq.t().matmul(wt) / scale
        cross_gradient_correction = Hblocks[0] - cross.t()

    if isinstance(Hblocks, torch.Tensor):
        st['hessian_blocks'] = Hblocks.cpu().to(torch.bfloat16)
        st['cross_gradient_correction'] = (
            cross_gradient_correction.cpu().to(torch.bfloat16)
        )
        st['cross_alpha'] = 0.5

    st['version'] = 'unified_full_hessian_cross_target'
    return {
        'weight_params': p,
        'activation_state': st,
    }
...
# 功能：在完整 H[k,k] 二次项与可选 cross-target 梯度下批量优化 mantissa/sign。
# 说明：每次只尝试 ±0.25 mantissa 步长；固定 gradient_bias 在增量更新中保持不变。
def _refine_full_hessian_batched(
    y, p, H, gradient_bias=None, iters=20, block_batch=4,
):
    ...
    if isinstance(gradient_bias, torch.Tensor):
        g.add_(gradient_bias)
    step = 0.25 * eff
...
# 功能：为 full-H hierarchy 初始化参数副本、当前反量化 q 与含 cross-target 的全局梯度。
# 说明：scale/lv3/lv2 共用该 q/g；参数变化仍只需增量更新 d@H。
def _full_hierarchy_init(y, p, H, gradient_bias=None):
...
# 功能：统一的动态 activation Full-H refinement，并可让 activation 补偿部分 weight 量化误差。
# 说明：只计算一次 alpha*y*(H-C^T)，mantissa/scale/lv3/lv2 全程共享相同 cross-target。
def _refine_activation_full_hessian(
    y, p, H, gradient_correction=None, *, cross_alpha=1.0,
    mantissa_iters=10, block_batch=4,
):
    ...
    gradient_bias = None
    if (
        isinstance(gradient_correction, torch.Tensor)
        and gradient_correction.numel() == k * k
        and float(cross_alpha) != 0.0
    ):
        correction = gradient_correction.to(
            y.device, torch.float32,
        ).reshape(k, k)
        gradient_bias = float(cross_alpha) * y.float().reshape(-1, k).matmul(
            correction,
        )
    p = _refine_full_hessian_batched(
        y, p, HH, gradient_bias=gradient_bias,
        iters=int(mantissa_iters), block_batch=int(block_batch),
    )

    full_state = _full_hierarchy_init(
        y, p, HH, gradient_bias=gradient_bias,
    )
...
# 功能：动态 Linear activation 量化入口：重放 geometry 后，以 Self-MSE 初始化并执行 cross-target Full-H refinement。
# 说明：cross correction 与 H 使用同一归一化；alpha=0.5 在输出精度与补偿强度间折中。
def hif4_dynamic_quantize_activation(aq, asc, st):
    ...
    return _refine_activation_full_hessian(
        y, p, Hblocks[0],
        gradient_correction=st.get('cross_gradient_correction'),
        cross_alpha=float(st.get('cross_alpha', 0.0)),
        mantissa_iters=10, block_batch=4,
    )

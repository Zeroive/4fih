...
def _merge_k_candidates_per_feature_block(x: torch.Tensor, params_list: list[dict], dq_list: list[torch.Tensor]) -> dict:
    """
...
def _select_best_k_dq_per_feature_block(x: torch.Tensor, dq_list: list[torch.Tensor]) -> torch.Tensor:
...
def _v35_quantize_k_tensor(x):
    ...
    for _ in range(1, _K_QUOTIENT_TOTAL_ROUNDS):
        ...
        for gamma in _K_RELAX_GAMMAS:
            p, q = _quantize_tensor_self_mse(x - (c_prev + float(gamma) * delta), return_dequant=True)
            params_list.append(p)
            dq_list.append(q)
...
def _v39_apply_head_perm(x: torch.Tensor, num_heads: int, head_dim: int, perm: Optional[torch.Tensor]) -> torch.Tensor:
...
def _v42_apply_head_matrix(x: torch.Tensor, num_heads: int, head_dim: int, mats: torch.Tensor) -> torch.Tensor:
...
def _v42_apply_v40_state_q(x, state, q_num_heads, head_dim):
    y = x.float()
    if not (isinstance(state, dict) and state.get('enabled', False)):
        return y
    s = state.get('scale')
    if isinstance(s, torch.Tensor) and s.numel() == y.shape[-1]:
        y = y * s.to(y.device)
    if state.get('packed', False):
        p = state.get('perm')
        if isinstance(p, torch.Tensor):
            y = _v39_apply_head_perm(y, q_num_heads, head_dim, p)
    else:
        r = int(state.get('rotation', -1))
        if r >= 0:
            y = _v35_rotate_heads(y, q_num_heads, head_dim, r)
    return y
...
def _v42_apply_v40_state_k(x, state, kv_num_heads, head_dim):
    ...
    if state.get('packed', False):
        p = state.get('perm')
        if isinstance(p, torch.Tensor):
            y = _v39_apply_head_perm(y, kv_num_heads, head_dim, p)
    else:
        r = int(state.get('rotation', -1))
        if r >= 0:
            y = _v35_rotate_heads(y, kv_num_heads, head_dim, r)
    return y
...
def _v44_apply_q(q, state, q_num_heads, head_dim):
    if state.get('version') == _V44_VERSION:
        if state.get('transform_kind') == 'rot':
            s = state.get('scale')
            q = q * s.to(q.device)
            q = _v35_rotate_heads(q, q_num_heads, head_dim, int(state.get('rotation', 0)))
        else:
            q = _v42_apply_head_matrix(q, q_num_heads, head_dim, state.get('matrix'))
    else:
        q = _v42_apply_v40_state_q(q, state, q_num_heads, head_dim)
    return q
...
def _v44_apply_k(k, state, kv_num_heads, head_dim):
    if state.get('version') == _V44_VERSION:
        if state.get('transform_kind') == 'rot':
            s = state.get('scale')
            k = k / s.to(k.device)
            k = _v35_rotate_heads(k, kv_num_heads, head_dim, int(state.get('rotation', 0)))
        else:
            k = _v42_apply_head_matrix(k, kv_num_heads, head_dim, state.get('matrix'))
    else:
        k = _v42_apply_v40_state_k(k, state, kv_num_heads, head_dim)
    return k
...
def _v37_quantize_hessian(x: torch.Tensor, hblocks: torch.Tensor, *, return_dequant=False):
    ...
    for rs in range(0, rows, chunk_rows):
        ...
        if dq_out is not None:
            dq_out[rs:re] = best_q.reshape(nr, nb, 8, 2, 4)
    prefix = shape[:-1]
    params = {'scale_factor': sf_out.reshape(*prefix, nb, 1, 1, 1), 'scale_lv2': l2_out.reshape(*prefix, nb, 8, 1, 1), 'scale_lv3': l3_out.reshape(*prefix, nb, 8, 2, 1), 'sign': sg_out.reshape(*prefix, nb, 8, 2, 4), 'mant': ma_out.reshape(*prefix, nb, 8, 2, 4)}
...
def _safe108_attach_cov_state(out, calib_qkv_list, q_num_heads, kv_num_heads, head_dim, use_q_h256=False, use_k_metric=False):
    ...
    if not decoded:
        return out
    qs = dict(out.get('q_state', {}))
    ks = dict(out.get('k_state', {}))
...
def _safe111_fixed_attention_base(calib_qkv_list, q_num_heads, kv_num_heads, head_dim):
    decoded = _v35_decode_calib(calib_qkv_list, q_num_heads, kv_num_heads, head_dim)
    common = {'version': _V44_VERSION, 'enabled': True, 'head_dim': int(head_dim), 'transform_kind': 'rot', 'rotation': 1, 'beta': 0.5, 'strict_safe_fixed': True}
    if not decoded:
        return {'q_state': {**common, 'role': 'q', 'scale': torch.ones(q_num_heads * head_dim)}, 'k_state': {**common, 'role': 'k', 'scale': torch.ones(kv_num_heads * head_dim)}, 'v_state': {'enabled': False, 'role': 'v', 'strict_safe_fixed': True}}
    sq, sk = _v35_qk_scale(decoded, q_num_heads, kv_num_heads, head_dim, 0.5)
    return {'q_state': {**common, 'role': 'q', 'scale': sq.detach().cpu().float()}, 'k_state': {**common, 'role': 'k', 'scale': sk.detach().cpu().float()}, 'v_state': {'enabled': False, 'role': 'v', 'strict_safe_fixed': True}}

def _safe111_attach(calib_qkv_list, q_num_heads, kv_num_heads, head_dim, use_q_h256=False, use_k_metric=False):
...
def _safe147_geometry(w, acts, perm_kind='v40', post_pressure='max', robust_phase_mix=0.0):
    ...
    phases = torch.exp((1.0 - mix) * torch.log(pmax.clamp_min(1e-24)) + mix * torch.log(prms.clamp_min(1e-24)))
...
def _v159_dynamic_k(k_quant, k_scale, kv_num_heads, head_dim, k_state, lv3_iters=1):
    ...
    H256 = k_state.get('partner_h256')
    H64 = k_state.get('partner_h64')
    ...
    if not isinstance(H64, torch.Tensor):
        return p0
    p1 = _v158_dynamic_tensor_h256(target, H64, H256d, lv3_iters=int(lv3_iters), base_mant_iters=1)
...
def hif4_dynamic_quantize_k(k_quant, k_scale, kv_num_heads, head_dim, k_state):
    return _v159_dynamic_k(k_quant, k_scale, kv_num_heads, head_dim, k_state, 0)
...
def hif4_calibration_and_quantize_weight(weight_quant, weight_scale, calib_activation_list):
    ...
    for key in ('weight_hessian_blocks', 'super256_hessian_blocks', 'super512_hessian_blocks', 'super1024_hessian_blocks', 'cross1024_hessian_blocks'):
        st.pop(key, None)
    st['weight_hessian_blocks'] = _safe90_hessian64(wq).cpu().to(torch.bfloat16)
    st['super256_hessian_blocks'] = _safe90_hessian256(wq).cpu().to(torch.bfloat16)
    st['super1024_hessian_blocks'] = _v173_weight_gram_blocks(wq, 1024).cpu().to(torch.bfloat16)
...
def hif4_dynamic_quantize_activation(aq, asc, st):
    if not isinstance(st, dict) or not isinstance(st.get('full2048_hessian'), torch.Tensor):
        return _quantize_nvfp4_operand(aq, asc)
    y = _safe90_decode_transform_activation(aq, asc, st)
    post = st.get('post_perm')
    if isinstance(post, torch.Tensor):
        y = y.index_select(-1, post.to(y.device, dtype=torch.long))
    H = st['full2048_hessian'].to(y.device, torch.float32)
    p = _v165_dynamic_activation(aq, asc, st)
    p = _v173_refine_group(y, p, st['super1024_hessian_blocks'].to(y.device, torch.float32), 1024, 1)
    return _v181b_pairblock_refine(y, p, H, 32, block_batch=4)

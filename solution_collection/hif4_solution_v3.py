...
# ============================================================
# V3: output-aware Smooth-QK search
# ============================================================
...
# Preserve the direct-attention implementation for unusual head layouts where
# Q heads cannot be mapped evenly to KV heads.
_hif4_calibration_attention_direct = hif4_calibration_attention

QK_BETA_CANDIDATES = (None, 0.25, 0.5, 0.75, 1.0)
QK_SEARCH_MAX_TOKENS = 64
...
def _qk_candidate(
    ...
    if beta is None or not USE_QK_SMOOTH:
        ...
        ratio = (q_abs + EPS) / (k_abs + EPS)
        s = ratio.pow(beta).clamp(*QK_SMOOTH_CLAMP)

    q_pre = (1.0 / s).repeat_interleave(q_per_kv, dim=0)
    k_pre = s
    Qt = Q * q_pre.unsqueeze(0)
    Kt = K * k_pre.unsqueeze(0)
    if USE_QK_HADAMARD64:
        Qt = _hadamard64(Qt)
        Kt = _hadamard64(Kt)

    q_second = Qt.square().mean(dim=0)
    k_second = Kt.square().mean(dim=0)
    qw = _normalize_importance(
        k_second.repeat_interleave(q_per_kv, dim=0).reshape(-1)
    )
    kw = _normalize_importance(
        q_second.reshape(kv_heads, q_per_kv, head_dim).sum(dim=1).reshape(-1)
    )
    Qflat, Kflat = Qt.flatten(1), Kt.flatten(1)
    Qq = _dequantize_hif4_params(_hif4_quantize_dynamic(Qflat, qw)).reshape_as(Qt)
    Kq = _dequantize_hif4_params(_hif4_quantize_dynamic(Kflat, kw)).reshape_as(Kt)
...
def hif4_calibration_attention(
    ...
    if q_num_heads % kv_num_heads != 0:
        return _hif4_calibration_attention_direct(

...
def _compute_weight_cross_blocks_64(wq, wt):
    """Build normalized H=Wq^T Wq and B=Wq^T Wt for each 64 block."""
    if wq.dim()!=2 or wt.dim()!=2 or wq.shape!=wt.shape or wq.shape[-1]%64:
        return None,None
    m,k=map(int,wq.shape);nb=k//64
    q=wq.float().reshape(m,nb,64);t=wt.float().reshape(m,nb,64)
    H=torch.einsum('mbi,mbj->bij',q,q)
    B=torch.einsum('mbi,mbj->bij',q,t)
    scale=H.diagonal(dim1=-2,dim2=-1).mean(-1).abs().clamp_min(1e-12)
    return H/scale[:,None,None],B/scale[:,None,None]
...
def _calibrate_and_quantize_linear_weight(weight_quant,weight_scale,calib_activation_list):
    ...
    wq=_dequantize_hif4_params(wp,tuple(w.shape)).float();Hc,B64=_compute_weight_cross_blocks_64(wq,wt)
    eye=torch.eye(64,device=w.device,dtype=torch.float32).unsqueeze(0)
    ridge=1.0e-3
    T64=torch.linalg.solve(Hc+ridge*eye,B64+ridge*eye)
    st=_make_quantized_weight_state('v169_cross_target',s,perm,had,ph,wq,{'post_perm':post.cpu().to(torch.int32),'post_perm_enabled':True,'rho64_mean':float(r64.mean()),'rho256_mean':float(r256.mean()),'weight_compensation_64':T64.detach().cpu().to(torch.bfloat16),'cross_target_lambda':0.25})
...
def _quantize_dynamic_activation(aq, asc, st):
    ...
    T64 = st.get('weight_compensation_64')
    if isinstance(T64,torch.Tensor) and y.shape[-1]%64==0:
        shape=y.shape;blocks=y.float().reshape(-1,y.shape[-1]//64,64)
        compensated=torch.einsum(
            'bij,rbj->rbi',T64.to(y.device,torch.float32),blocks
        ).reshape(shape)
        lam=float(st.get('cross_target_lambda',0.25))
        y=y+lam*(compensated-y)
...
_ATTENTION_STATE_VERSION = "v171_extreme_robust_signed_seed2"
...
def _rotate_heads_by_pattern(
    ...
    if bool((patterns == patterns[0]).all()):
        return _rotate_feature_blocks_64(y, int(patterns[0])).reshape(orig_shape)

    out = y
    for pattern in torch.unique(patterns).tolist():
        if int(pattern) < 0:
            continue
        transformed = _rotate_feature_blocks_64(y, int(pattern))
        mask = (patterns == int(pattern)).reshape(1, num_heads, 1)
        out = torch.where(mask, transformed, out)

    return out.reshape(orig_shape)

def _decode_attention_calibration(calib_qkv_list):
    decoded = []
    for sample in calib_qkv_list:
        try:
            decoded.append(tuple(
                dequantize_nvfp4(*sample[role]).float()
                for role in ("q", "k", "v")
            ))
        except Exception:
            continue
...
def _compute_reciprocal_qk_scales(
    decoded, q_num_heads, kv_num_heads, head_dim, beta,
):
    device = decoded[0][0].device
    queries_per_kv = q_num_heads // kv_num_heads
    q_max = torch.zeros((q_num_heads, head_dim), device=device)
    k_max = torch.zeros((kv_num_heads, head_dim), device=device)
    for q, k, _ in decoded:
        q_heads = q.reshape(-1, q_num_heads, head_dim)
        k_heads = k.reshape(-1, kv_num_heads, head_dim)
        q_max = torch.maximum(q_max, q_heads.abs().amax(dim=0))
        k_max = torch.maximum(k_max, k_heads.abs().amax(dim=0))
    if beta <= 0.0:
        k_scale = torch.ones_like(k_max)
    else:
        q_group_max = q_max.reshape(
            kv_num_heads, queries_per_kv, head_dim,
        ).amax(dim=1)
        log_scale = float(beta) * (
            torch.log(k_max.clamp_min(2.0 ** -24))
            - torch.log(q_group_max.clamp_min(2.0 ** -24))
        )
        log_scale -= log_scale.median(dim=-1, keepdim=True).values
        k_scale = torch.exp(log_scale).clamp_(2.0 ** -6, 2.0 ** 6)
    q_scale = k_scale.repeat_interleave(queries_per_kv, dim=0)
    return q_scale.reshape(-1), k_scale.reshape(-1)

def _normalize_hessian_blocks(hessian):
    hessian = hessian.float()
    scale = hessian.diagonal(dim1=-2, dim2=-1).mean(-1).abs().clamp_min(1e-12)
    return hessian / scale[:, None, None]

def _extract_hessian_64_blocks(hessian_256):
    blocks = []
    for head in range(int(hessian_256.shape[0])):
        for start in range(0, int(hessian_256.shape[-1]), 64):
            blocks.append(hessian_256[head, start:start + 64, start:start + 64])
    return torch.stack(blocks, dim=0)

def _compute_attention_partner_covariances(
    decoded, q_state, k_state, q_num_heads, kv_num_heads, head_dim,
):
    if not decoded or head_dim % 64:
        return None
    queries_per_kv = q_num_heads // kv_num_heads
    device = decoded[0][0].device
    q_partner = torch.zeros(
        (kv_num_heads, head_dim, head_dim), device=device, dtype=torch.float32,
    )
    k_partner = torch.zeros_like(q_partner)
    k_count = 0
    q_count = 0
    for q, k, _ in decoded:
        q_transformed = _apply_query_transform(
            q.float(), q_state, q_num_heads, head_dim,
        ).reshape(-1, q_num_heads, head_dim)
        k_transformed = _apply_key_transform(
            k.float(), k_state, kv_num_heads, head_dim,
        ).reshape(-1, kv_num_heads, head_dim)
        k_centered = k_transformed - k_transformed.mean(dim=0, keepdim=True)
        q_partner.add_(torch.einsum('shd,she->hde', k_centered, k_centered))
        q_grouped = q_transformed.reshape(
            q_transformed.shape[0], kv_num_heads, queries_per_kv, head_dim,
        )
        k_partner.add_(torch.einsum('shrd,shre->hde', q_grouped, q_grouped))
        k_count += int(k_centered.shape[0])
        q_count += int(q_grouped.shape[0]) * int(queries_per_kv)
    if k_count <= 0 or q_count <= 0:
        return None
    q_partner = _normalize_hessian_blocks(q_partner / float(k_count))
    k_partner = _normalize_hessian_blocks(k_partner / float(q_count))
    q_h256 = q_partner.repeat_interleave(queries_per_kv, dim=0).contiguous()
    k_h256 = k_partner.contiguous()
    return (
        _extract_hessian_64_blocks(q_h256), q_h256,
        _extract_hessian_64_blocks(k_h256), k_h256,
    )

def _attach_attention_covariance_state(
    states, decoded, q_num_heads, kv_num_heads, head_dim,
):
    q_state = dict(states["q_state"])
    k_state = dict(states["k_state"])
    covariance = _compute_attention_partner_covariances(
        decoded, q_state, k_state, q_num_heads, kv_num_heads, head_dim,
    )
    if covariance is None:
        return states
    q_h64, q_h256, k_h64, k_h256 = covariance
    q_state.update({
    ...
    return {**states, "q_state": q_state, "k_state": k_state}

def _score_key_reconstruction_by_head(
    target, dequantized, hessian_256, kv_num_heads, head_dim,
):
    error = (dequantized.float() - target.float()).reshape(
        target.shape[-2], kv_num_heads, head_dim,
    )
    error -= error.mean(dim=0, keepdim=True)
    hessian = hessian_256.to(target.device, torch.float32)
    return torch.einsum('shd,hde,she->h', error, hessian, error)

def _merge_key_params_by_head(params_list, best_head, kv_num_heads, head_dim):
    if len(params_list) == 1:
        return params_list[0]
    best_blocks = best_head.repeat_interleave(head_dim // 64)
    merged = {}
    for name, base in params_list[0].items():
        value = base.clone()
        tail_dims = base.dim() - 2
        for candidate_idx in range(1, len(params_list)):
            mask = (best_blocks == candidate_idx).reshape(
                1, -1, *([1] * tail_dims),
            )
            value = torch.where(mask, params_list[candidate_idx][name], value)
        merged[name] = value
    return merged

def _quantize_key_with_partner_metric(
    target, hessian_256, kv_num_heads, head_dim,
):
    target = target.float()
    if target.dim() != 2 or target.shape[-2] <= 1:
        return _quantize_key_fast_translation_search(target)
    params_list = []
    dequantized_list = []

    def add_candidate(candidate):
        params, dequantized = _quantize_tensor_self_mse(
            candidate, return_dequant=True,
        )
        params_list.append(params)
        dequantized_list.append(dequantized)

    def current_best():
        scores = torch.stack([
            _score_key_reconstruction_by_head(
                target, value, hessian_256, kv_num_heads, head_dim,
            )
            for value in dequantized_list
        ])
        best = scores.argmin(dim=0)
        candidates = torch.stack([
            value.reshape(target.shape[0], kv_num_heads, head_dim)
            for value in dequantized_list
        ])
        mixed = candidates[0].clone()
        for candidate_idx in range(1, len(dequantized_list)):
            mask = (best == candidate_idx).reshape(1, kv_num_heads, 1)
            mixed = torch.where(mask, candidates[candidate_idx], mixed)
        return best, mixed.reshape_as(target)

    add_candidate(target)
    best, best_dequantized = current_best()
    previous_center = torch.zeros_like(target.mean(dim=-2, keepdim=True))
    for _ in range(_KEY_MEAN_REFINEMENT_ROUNDS):
        center = (target - best_dequantized).mean(dim=-2, keepdim=True)
        delta = center - previous_center
        for factor in _KEY_TRANSLATION_STEP_FACTORS:
            add_candidate(target - (previous_center + float(factor) * delta))
        best, best_dequantized = current_best()
        previous_center = center

    previous_center = target.median(dim=-2, keepdim=True).values
    add_candidate(target - previous_center)
    best, best_dequantized = current_best()
    for _ in range(_KEY_MEDIAN_REFINEMENT_ROUNDS):
        center = (target - best_dequantized).mean(dim=-2, keepdim=True)
        delta = center - previous_center
        ...
        best, best_dequantized = current_best()
        previous_center = center
    return _merge_key_params_by_head(
        params_list, best, kv_num_heads, head_dim,
    )

def _quantize_dynamic_query_v165(
    q_quant, q_scale, q_num_heads, head_dim, q_state, lv3_iters=1,
):
    query = dequantize_nvfp4(q_quant, q_scale).float()
    if isinstance(q_state, dict):
        query = _apply_query_transform(query, q_state, q_num_heads, head_dim)
        hessian_64 = q_state.get("partner_h64")
        hessian_256 = q_state.get("partner_h256")
        if isinstance(hessian_64, torch.Tensor):
            return _quantize_dynamic_tensor_hessian(
                query, hessian_64, hessian_256,
                lv3_iters=int(lv3_iters), base_mant_iters=1,
            )
    return _quantize_tensor_self_mse(query, return_dequant=False)[0]

def _quantize_dynamic_key_v165(
    k_quant, k_scale, kv_num_heads, head_dim, k_state, lv3_iters=1,
):
    key = dequantize_nvfp4(k_quant, k_scale).float()
    if not isinstance(k_state, dict):
        return _quantize_key_fast_translation_search(key)
    transformed = _apply_key_transform(key, k_state, kv_num_heads, head_dim)
    hessian_256 = k_state.get("partner_h256")
    hessian_64 = k_state.get("partner_h64")
    if not isinstance(hessian_256, torch.Tensor):
        return _quantize_key_fast_translation_search(transformed)
    hessian_256 = hessian_256.to(transformed.device, torch.float32)
    baseline = _quantize_key_with_partner_metric(
        transformed, hessian_256, kv_num_heads, head_dim,
    )
    baseline_dequantized = _dequantize_hif4_params(
        baseline, tuple(transformed.shape),
    ).float()
    centered_target = transformed - (
        transformed - baseline_dequantized
    ).mean(dim=-2, keepdim=True)
    if not isinstance(hessian_64, torch.Tensor):
        return baseline
    refined = _quantize_dynamic_tensor_hessian(
        centered_target, hessian_64, hessian_256,
        lv3_iters=int(lv3_iters), base_mant_iters=1,
    )
    refined_dequantized = _dequantize_hif4_params(
        refined, tuple(transformed.shape),
    ).float()
    baseline_score = _score_key_reconstruction_by_head(
        transformed, baseline_dequantized, hessian_256,
        kv_num_heads, head_dim,
    )
    refined_score = _score_key_reconstruction_by_head(
        transformed, refined_dequantized, hessian_256,
        kv_num_heads, head_dim,
    )
    best = (refined_score < baseline_score).to(torch.long)
    return _merge_key_params_by_head(
        [baseline, refined], best, kv_num_heads, head_dim,
    )
...
def hif4_calibration_attention(
    ...
    """V165 fixed signed-H64 attention with reciprocal Smooth and covariance."""
    ...
    decoded = _decode_attention_calibration(calib_qkv_list)
    if decoded:
        q_scale, k_scale = _compute_reciprocal_qk_scales(
            decoded, q_num_heads, kv_num_heads, head_dim, beta=0.5,
        )
    else:
        q_scale = torch.ones(q_num_heads * head_dim, dtype=torch.float32)
        k_scale = torch.ones(kv_num_heads * head_dim, dtype=torch.float32)
    common = {
    ...
    states = {
        "q_state": {
            **common,
            ...
            "head_rotation_patterns": torch.full(
                (q_num_heads,), 2, dtype=torch.int8,
            ),
        },
        "k_state": {
            **common,
            ...
            "head_rotation_patterns": torch.full(
                (kv_num_heads,), 2, dtype=torch.int8,
            ),
        },
        "v_state": {
            "version": _ATTENTION_STATE_VERSION,
    ...
    return _attach_attention_covariance_state(
        states, decoded, q_num_heads, kv_num_heads, head_dim,
    )
...
def hif4_dynamic_quantize_q(
    ...
    return _quantize_dynamic_query_v165(
        q_quant,
        q_scale,
        q_num_heads,
        head_dim,
...
def hif4_dynamic_quantize_k(
    ...
    return _quantize_dynamic_key_v165(
        k_quant,
        k_scale,
        kv_num_heads,
        head_dim,

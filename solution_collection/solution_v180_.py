...
# 功能：将每个 KV group 的公共通道 permutation 应用到其 K head 和关联 Q heads。
# 说明：同组 Q/K 使用同一置换，因此标准 GQA 点积保持不变。
def _apply_attention_group_permutation(x, state, num_heads, head_dim):
    perm = state.get('group_perm')
    if not isinstance(perm, torch.Tensor) or perm.dim() != 2 or int(perm.shape[-1]) != head_dim:
        return x.float()
    kv_heads = int(perm.shape[0])
    if kv_heads <= 0 or num_heads % kv_heads != 0:
        return x.float()
    per_head = perm.to(x.device, dtype=torch.long).repeat_interleave(num_heads // kv_heads, dim=0)
    y = x.float().reshape(-1, num_heads, head_dim)
    return y.gather(2, per_head.unsqueeze(0).expand(y.shape[0], -1, -1)).reshape_as(x)
...
def _apply_q_transform(q, state, q_num_heads, head_dim):
    ...
    q = _apply_attention_group_permutation(q, state, q_num_heads, head_dim)
...
def _apply_k_transform(k, state, kv_num_heads, head_dim):
    ...
    k = _apply_attention_group_permutation(k, state, kv_num_heads, head_dim)
    return _v35_rotate_heads(
...
def hif4_calibration_attention(calib_qkv_list, q_num_heads, kv_num_heads, head_dim):
    ...
    rep = q_num_heads // kv_num_heads
    q_energy = torch.zeros((kv_num_heads, head_dim), device=decoded[0][0].device)
    ...
    common = {
        ...
        'rotation': 2,
        'beta': 0.5,
        'group_perm': group_perm.detach().cpu().to(torch.int32),

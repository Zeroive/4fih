...
def _v44_apply_q(q, state, q_num_heads, head_dim):
    ...
    if isinstance(patterns, torch.Tensor):
        ...
        q = _v162_rotate_heads_by_pattern(
            q,
            q_num_heads,
            head_dim,
            patterns,
...
def _v44_apply_k(k, state, kv_num_heads, head_dim):
    ...
    if isinstance(patterns, torch.Tensor):
        ...
        k = _v162_rotate_heads_by_pattern(
            k,
            kv_num_heads,
            head_dim,
            patterns,
...
# =============================================================================
# V162: per-KV-head Q/K rotation selection
#
# For each KV head independently choose:
#   -1: identity
#    0: H64
#    1: signed H64
#
# All Q heads associated with one KV head share the same rotation,
# therefore QK^T is exactly preserved.
#
# Everything after the transform keeps the V159 Attention path:
#   Q: partner-covariance H64/H256 refinement
#   K: partner-covariance quotient + hierarchy refinement
#   V: V113 tensor-self path
# =============================================================================

_V162_VERSION = "v162_per_head_rotation"
_V162_HEAD_ROTATION_CANDIDATES = (-1, 0, 1)

def _v162_rotate_heads_by_pattern(
    x: torch.Tensor,
    num_heads: int,
    head_dim: int,
    ...
    #
    # _v35_rotate64 treats the last dimension independently, therefore
    # [rows, heads, head_dim] is safe: heads never mix.
    h64 = _v35_rotate64(y, 0)
    sh64 = _v35_rotate64(y, 1)
...
def _v162_choose_head_rotations(
    decoded,
    q_num_heads: int,
    kv_num_heads: int,
    head_dim: int,
    ...
    candidates = _V162_HEAD_ROTATION_CANDIDATES
    nc = len(candidates)
...
def hif4_calibration_attention(
    ...
    #
    # Because _safe108_attach_cov_state internally calls
    # _v44_apply_q/_k, and we already taught those functions
    # how to recognize head_rotation_patterns, the covariance
    # is calculated AFTER the selected per-head transforms.
    # ---------------------------------------------------------
    out = _safe108_attach_cov_state(
        out,
        calib_qkv_list,
        q_num_heads,
        kv_num_heads,
...
# =============================================================================
# V162 final public Attention API
# =============================================================================

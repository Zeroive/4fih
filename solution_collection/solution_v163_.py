...
def _decode_attention_calibration(calib, q_num_heads, kv_num_heads, head_dim):
    out = []
    for s in calib:
        try:
...
def _compute_reciprocal_qk_scales(decoded, q_num_heads, kv_num_heads, head_dim, beta):
...
def _normalize_hessian_blocks(H):
    H=H.float()
    sc=H.diagonal(dim1=-2,dim2=-1).mean(-1).abs().clamp_min(1e-12)
    return H/sc[:,None,None]

def _extract_hessian_64_blocks(H256):
...
def _compute_attention_partner_covariances(decoded,qs,ks,q_num_heads,kv_num_heads,head_dim):
    ...
    for q,k,_ in decoded:
        ...
        kt=_apply_key_transform(k.float(),ks,kv_num_heads,head_dim).reshape(-1,kv_num_heads,head_dim)
...
def _score_key_reconstruction_by_head(x,dq,H256,kv_num_heads,head_dim):
...
def _quantize_key_with_partner_metric(x,H256,kv_num_heads,head_dim):
    """
    ...
    if x.dim()!=2 or int(x.shape[-2])<=1 or not isinstance(H256,torch.Tensor):
        return _quantize_key_fast_translation_search(x)
    ...
    def current_best():
        scores=torch.stack([_score_key_reconstruction_by_head(x,z,H256,kv_num_heads,head_dim) for z in dqs],0)
    ...
    return _merge_key_params_by_head(params,best,kv_num_heads,head_dim)
...
def _calibrate_and_quantize_linear_weight(weight_quant,weight_scale,calib_activation_list):
    ...
    if isinstance(H256,torch.Tensor):wp=_refine_weight_hessian_256_chunked(wt,wp,H256,iters=8,chunk_rows=256)
...
def _quantize_dynamic_key(k_quant,k_scale,kv_num_heads,head_dim,k_state,lv3_iters=1):
    ...
    x=_apply_key_transform(k,k_state,kv_num_heads,head_dim)
    ...
    p1=_quantize_dynamic_tensor_hessian(
        target,H64,H256d,lv3_iters=int(lv3_iters),base_mant_iters=1
    )
    q1=_dequantize_hif4_params(p1,tuple(x.shape)).float()
    s0=_score_key_reconstruction_by_head(x,q0,H256d,kv_num_heads,head_dim)
    s1=_score_key_reconstruction_by_head(x,q1,H256d,kv_num_heads,head_dim)
    best=(s1<s0).to(torch.long)
    return _merge_key_params_by_head([p0,p1],best,kv_num_heads,head_dim)
...
def _quantize_dynamic_query(q_quant,q_scale,q_num_heads,head_dim,q_state,lv3_iters=1):
    ...
    if isinstance(q_state,dict):
        ...
        if isinstance(H64,torch.Tensor):
            return _quantize_dynamic_tensor_hessian(q,H64,H256,lv3_iters=int(lv3_iters),base_mant_iters=1)
...
_ATTENTION_STATE_VERSION = "v162_per_head_rotation"

_HEAD_ROTATION_CANDIDATES = (-1, 0, 1)
...
def _choose_head_rotation_patterns(
    decoded,
    q_num_heads: int,
    kv_num_heads: int,
    head_dim: int,
    ...
    """
    Independently choose rotation for each KV head.
    ...
    All Q heads belonging to one KV head share the same candidate.
    """
    ...
    candidates = _HEAD_ROTATION_CANDIDATES
    nc = len(candidates)

    # No meaningful H64 search when a head cannot be split into 64-blocks.
    if head_dim % 64 != 0:
        k_patterns = torch.full(
            (kv_num_heads,),
            -1,
            dtype=torch.long,
        )
        q_patterns = k_patterns.repeat_interleave(rep)
        return q_patterns, k_patterns

    device = decoded[0][0].device

    sqh = sq.to(device).reshape(
        q_num_heads,
        head_dim,
    )

    skh = sk.to(device).reshape(
        kv_num_heads,
        head_dim,
    )
    ...
    k_error = torch.zeros_like(q_error)

    # Energy does not depend on orthogonal rotation.
    q_energy = torch.zeros(
        kv_num_heads,
        dtype=torch.float64,
        device=device,
    )

    k_energy = torch.zeros_like(q_energy)
    ...
    for q, k, _ in decoded:
        ...
        for kv_head in range(kv_num_heads):
            ...
            # Associated Q heads
            q_group = qh[:, q_begin:q_end, :]

            # One KV head
            k_head = kh[:, kv_head, :]

            # -----------------------------------------------
            # Apply reciprocal Smooth first.
            #
            # Q' = Q * s
            # K' = K / s
            # -----------------------------------------------
            q_group = (
                q_group
                * sqh[q_begin:q_end, :].unsqueeze(0)
            )

            k_head = (
                k_head
                / skh[kv_head, :].unsqueeze(0)
            )

            q_energy[kv_head] += (
                q_group.double().square().sum()
            )

            k_energy[kv_head] += (
                k_head.double().square().sum()
            )
            ...
            #    1 signed H64
            # -----------------------------------------------
            for ci, pattern in enumerate(candidates):
                qt = _rotate_feature_blocks_64(
                    q_group,
                    pattern,
                )

                kt = _rotate_feature_blocks_64(
                    k_head,
                    pattern,
                )
                ...
                # Important:
                # q_group shape =
                #   [seq, Q-heads-per-KV, head_dim]
                #
                # last dim == head_dim, therefore HiF4
                # 64-block never crosses Q heads.
                _, qdq = _quantize_tensor_self_mse(
                    qt,
                    return_dequant=True,
                )

                _, kdq = _quantize_tensor_self_mse(
                    kt,
                    return_dequant=True,
                )

                q_error[kv_head, ci] += (
                    (qdq.double() - qt.double())
                    .square()
                    .sum()
                )

                k_error[kv_head, ci] += (
                    (kdq.double() - kt.double())
                    .square()
                    .sum()
                )
    ...
    q_rel = (
        q_error
        / q_energy.clamp_min(eps).unsqueeze(1)
    )

    k_rel = (
        k_error
        / k_energy.clamp_min(eps).unsqueeze(1)
    )

    # Equal Q/K relative-error weighting.
    score = q_rel + k_rel
    ...
    # One pattern per KV head.
    k_patterns = candidate_tensor[best]

    # GQA:
    # every associated Q head uses the same rotation.
    q_patterns = k_patterns.repeat_interleave(rep)
...
def hif4_calibration_attention(
    ...
    # ---------------------------------------------------------
    # No calibration samples: preserve previous safe default.
    # ---------------------------------------------------------
    if not decoded:
        q_patterns = torch.ones(
            q_num_heads,
            dtype=torch.long,
        )

        k_patterns = torch.ones(
            kv_num_heads,
            dtype=torch.long,
        )
        ...
        out = {
            "q_state": {
                ...
                "head_rotation_patterns":
                    q_patterns.to(torch.int8),
            },
    ...
    # ---------------------------------------------------------
    # Keep V113 reciprocal Q/K Smooth unchanged:
    #
    # Q' = Q * sq
    # K' = K / sk
    #
    # beta = 0.5
    # ---------------------------------------------------------
    sq, sk = _compute_reciprocal_qk_scales(
        decoded,
        q_num_heads,
        kv_num_heads,
        head_dim,
    ...
    out = {
        "q_state": {
            **common,
            "role": "q",
            "scale": sq.detach().cpu().float(),
            "head_rotation_patterns":
                q_patterns.detach().cpu().to(torch.int8),
        },

        "k_state": {
            **common,
            "role": "k",
            "scale": sk.detach().cpu().float(),
            "head_rotation_patterns":
                k_patterns.detach().cpu().to(torch.int8),
        },
    ...
    # ---------------------------------------------------------
    # Keep V113 partner-covariance machinery.
    #
    # The covariance helper applies the query/key transforms and recognizes
    # ``head_rotation_patterns``, so covariance is measured after the selected
    # per-head transforms.
    # ---------------------------------------------------------
    out = _attach_attention_covariance_state(
        out,
        calib_qkv_list,
        q_num_heads,
        kv_num_heads,
